 ...
 *
 * Устройство ставится на ноги: в исходнике оно лежит вверх ногами,
 * здесь координаты поворачиваются на 180° вокруг Z.
 *
...
for (let t = 0; t < count; t++) {
  for (let v = 0; v < 3; v++) {
    const p = vert(t, v);
    for (let a = 0; a < 3; a++) {
      // Поворот на 180° вокруг оси Z: в исходном STL устройство лежит
      // вверх ногами — разъёмы уходят вниз, фонарь вверх, кнопка
      // оказывается над индикаторами. Поворот (а не отражение) ставит
      // его на ноги и оставляет лицевую сторону смотреть в +Z.
      const flip = a < 2 ? -1 : 1;
      const n = (((p[a] - centre[a]) / span) * 2) * flip;
