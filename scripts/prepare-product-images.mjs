 ...
 * только перевод в webp. Качество выше общего — пиксели объекта отдаются
 * такими, какими их прислали.
 *
 * Пустые прозрачные поля при этом срезаются, как у всех остальных: без
 * этого объект занимал 88 %×50 % кадра против 100 % у прочих пакшотов
 * и выходил в плитке заметно мельче соседних. Обрезка полей — это рамка,
 * а не обработка снимка.
 *
 * Список нужен, чтобы правка пережила пересборку: без него следующий
 * прогон снова прогнал бы кадр через defringe.
 */
const AS_IS = new Map([["joim-es29-set", 92]]);
