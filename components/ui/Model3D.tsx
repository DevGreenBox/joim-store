...
void main() {
  ...
  // поэтому хватает узкой полосы у самого края. Внутри поля панели
  // наклон не спрашиваем: там стоит планка поперёк ниши, и её боковые
  // стенки иначе берут цвет канта — поперёк панели ложилась зелёная
  // щепка. Фаска корпуса за поле выходит и зелёной остаётся.
  bool inPanel = abs(vLocal.x) <= uHalf.x * 0.63 && abs(vLocal.z) <= uHalf.z * 0.53;
  bool faceY = abs(vLocal.y) >= uHalf.y * 0.93 && (abs(nLocal.y) >= 0.85 || inPanel);
  if (faceZ) {
